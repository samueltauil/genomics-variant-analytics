...
The workflow and local gates are implemented and tested, but no Azure
federated identity, ACR endpoint, or reviewer identity was supplied, so no
Azure resource was provisioned, no image was pushed, and no release was
published. The exact live checks for OIDC login, approved deployment records,
ACR attestations, and immutable release behavior therefore remain blocked
