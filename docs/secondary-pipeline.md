# Secondary-analysis pipeline (local/containerized)

This page documents the portable Nextflow secondary-analysis pipeline
delivered for tasks 5.1, 5.5, and 5.6 of the secondary-analysis processing
spec (`openspec/changes/add-genomics-variant-accelerator/specs/processing/secondary-analysis/spec.md`).
...
Tasks 5.2-5.4 and 5.7 (BWA/GATK-equivalent tool selection, Azure Batch/Slurm
live execution, cost/performance benchmarking, and further pipeline
hardening) are out of this scope and are not claimed as complete here.
...
## Azure Batch and Slurm profiles -- configuration preparation only

`workflows/conf/azure_batch.config` and `workflows/conf/slurm.config` define
executor profiles for Azure Batch and Slurm, per this task's explicit scope
of "configuration preparation" rather than live execution:

- Both profiles are credential-free and managed-identity-oriented: the Azure
  Batch profile references a pool/account by name/environment variable only
  and relies on managed identity for authentication, never an embedded key
  or connection string.
- Neither profile was exercised against a live Azure Batch account or Slurm
  cluster in this delivery. No Azure spend occurred for this work, and none
  was required, despite the user's authorized ceiling for this session.
- Validating these profiles against a real Batch pool or Slurm cluster is a
  distinct, not-yet-completed activity (tracked separately from tasks 5.1/
  5.5/5.6).
