...
complete. Reports use schema version 4 and `completeness_evaluated: true`.
...
Stability alone is never a failure signal: a paused or truncated file remains
`arriving` until the sender records an authoritative terminal failure. Configure
the scanner with a root-relative transfer manifest for expected sizes and a
root-relative failure marker for terminal failures:

```powershell
python scripts/scan_landing.py --root "$env:TEMP\synthetic-landing" --inventory "$env:TEMP\failure-inventory.sqlite3" --transfer-manifest '{run_id}/transfer-manifest.json' --failure-marker '{run_id}/transfer-failed.json'
```

The manifest template must use `{run_id}` exactly once so a manifest is still
found when none of its files arrived. Each manifest is a JSON object with
exactly `run_id` and `files`, where every entry has exactly `path` and
`size_bytes`:
...
The expected size prevents a short file from becoming `complete`; it never turns
elapsed time or stability into failure.

The failure marker is the authoritative transfer-session terminal signal. It is
a JSON object with exactly `run_id`, `status: "failed"` and `files`; each file
entry has exactly `path`, a non-empty `reason`, and the failed-generation
fingerprint `size_bytes` and `modified_ns` (both set for an observed file, both
null for a file that never arrived):
  ...
  "files": [
    {
      ...
      "size_bytes": 209715200,
      "modified_ns": 1789830000000000000
    }
  ]
}
```

The marker must be newer than the failed generation. An invalid marker is
reported in `failure_marker_errors` and holds the run at
`metadata_error: failure-marker-unavailable`; a marker whose fingerprint no
longer matches is stale and cannot fail a retried generation.

`unchanged_since` records when the current size and modification time were first
seen. Against the declared size and authoritative marker:

| Observation | State | `failure_reason` |
| --- | --- | --- |
| Larger than declared | `failed` | `size-exceeds-declared` |
| Short or absent without a terminal failure marker | `arriving` | none |
| Marker matches the observed failed generation | `failed` | marker `reason` |
| Marker identifies a file that never arrived | `failed` | marker `reason` |
| Equal to declared | `complete` once stable or marked | none |

Declared files and marker-only files that never arrive are listed with
`present: false` and the run and sample identifiers their declared path
resolves to, so the failure names the affected run, sample and file.

Retry needs no separate command. A re-sent file changes size or modification
time, which makes the old marker stale, resets `unchanged_since`, clears
`failure_reason` and returns the existing record to `arriving`; it becomes
`complete` once it reaches the declared size and stabilizes. The record is
revised in place, so the retry replaces the failed entry and keeps the original
`arrival_timestamp`. Sibling files of the same run are evaluated independently
...
Two unchanged observations are a completeness heuristic, not proof of transfer
success. Without an authoritative failure marker, the scanner therefore
classifies no failure and reports `failure_detection: not-evaluated`. The
guarantee is bounded by the sender's expected-size declaration and terminal
failure record: a sender that declares or fingerprints the wrong generation can
still produce an incorrect result. Marker authenticity and content-level
corruption that preserves size are outside this metadata-only check. Staging and
cloud scheduling remain pending. Do not use this inventory as an authorization
to stage data; `azure_readiness` stays `not-evaluated`.
...
configured transfer manifest or failure marker, each bounded at 1 MiB and parsed
as strict JSON. A traversal error discards that poll and leaves the prior
snapshot intact.
...
The landing tests generate tiny synthetic fixtures in temporary directories. They
...
mtime-only changes, vendor markers, transfer-manifest validation, authoritative
failure markers, invalid and stale markers, interrupted and absent transfers,
retry, sibling independence, staging exclusion and the CLI. No sequencing files
or inventory databases are stored in repository fixtures.