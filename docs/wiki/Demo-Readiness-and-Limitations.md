...
**There is still no runnable end-to-end demo as of 2026-09-19.** Repository guardrails, ingestion, staging, reference publication, the secondary-analysis pipeline, the Delta variant store, metadata lineage, governance, analytics/visualization, and the engineering-platform controls all have local synthetic or measured-cloud acceptance evidence. OIDC-backed ACR publication and registry-hosted provenance/SBOM verification are now live. Batch/Slurm execution, Managed Lustre scratch, the private governed query path, independent protected-environment approval, and the live seven-step presentation remain unverified or unimplemented.
...
| Staging to object storage (3.1, 3.2, 3.3) | Data Factory Copy over private endpoints stages only complete files; a growing file was skipped. Checksums are compared and a corrupted destination is marked failed and withheld; the staging record carries all six required fields. An account-native lifecycle policy is deployed, but its synthetic acceptance object still reports `Hot`. Purview lineage (3.4) and the completed lifecycle transition observation (3.5) remain absent. |
...
| Engineering platform (10.1, 10.3-10.11 except 10.2) | Immutable release `v0.2.1-pipeline` authenticated to Azure through OIDC, pushed its image to the admin-disabled ACR, and published verified registry-backed SLSA provenance and CycloneDX SBOM attestations. A stored synthetic variant resolves through the release and image digest to the signed tool versions. Attestation is enforced in the submission path; repository AI context, automated PR review, scheduled triage, and account-tier substitutes are also implemented. The `clinical` and `research` environments are configured, but independent-review and approved-deployment evidence for 10.2 is absent. |
| Preflight, reset, teardown, and cost measurements | Local snapshot preflight and named-store reset diagnostics are implemented and tested. The earlier environment was torn down on 2026-09-18; the fresh tagged foundation reached its documented ready state and has a dated size-based cost statement. Complete-environment no-change reporting remains unverified. |

The development task record is **80/89 completed**. Remaining open tasks are Purview lineage and lifecycle tiering (3.4, 3.5), Batch/Slurm/Lustre execution and concordance (5.2-5.4, 5.7), the private governed query path (8.7), independently approved environment deployment (10.2), and the full seven-step demo run (12.1).

The 2026-09-10/11 disposable environment was billable while it ran and was removed on 2026-09-18. A separate resource group, `rg-genomics-20260919`, exists in `eastus2` with an October 3, 2026 expiry tag. Its verification VM is deallocated, but storage and networking resources remain billable. The private foundation, account-native lifecycle policy, and ACR deployments succeeded. The synthetic lifecycle object still reports `Hot`, while the governed query endpoint and full demo remain incomplete. Deployment has been exercised only in the current sandbox subscription, so regional availability, quota and policy differences should be expected elsewhere. The dated `eastus2` cost model covers the deployed foundation sizing; Batch, Managed Lustre, analytics capacity, payload processing, and actual cost per sample remain excluded.
