...
The development task record is **79/89 completed**. Remaining open tasks are Purview lineage and lifecycle tiering (3.4, 3.5), Batch/Slurm/Lustre execution and concordance (5.2-5.4, 5.7), the private governed query path (8.7), independently approved environment deployment (10.2), complete-environment no-change reporting (11.4), and the full seven-step demo run (12.1).

The 2026-09-10/11 disposable environment was billable while it ran and was removed on 2026-09-18. A separate fresh resource group, `rg-genomics-20260919`, now exists in `eastus2` with an October 3, 2026 expiry tag. Its verification VM is deallocated, but storage and networking resources remain billable. The private foundation and ACR deployments succeeded, while the Storage Actions transition, governed query endpoint, and full demo remain incomplete. Deployment has been exercised only in the current sandbox subscription, so regional availability, quota and policy differences should be expected elsewhere. The dated `eastus2` cost model covers the deployed foundation sizing; Batch, Managed Lustre, analytics capacity, payload processing, and actual cost per sample remain excluded.
