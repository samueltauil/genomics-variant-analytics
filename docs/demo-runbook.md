...
verification-client, and account-native lifecycle-policy deployments report
`Succeeded`.
...
| 5. Query | Gene, quality, cross-cohort, allele-in-sample, pipeline-version, and sequencing-run results with traceability | **Local:** governed SQLite query engine, notebook-style session, SQL adapter, snapshots and traceability. **Blocked live:** the private Azure environment has no deployed notebook, SQL warehouse, or governed query endpoint, so private query acceptance cannot be claimed. |
