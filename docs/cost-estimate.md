...
`infra/main.bicep` deploys by default, now including the Storage Actions
lifecycle task added for task 3.5. It does not make a claim about a full
...
| Storage Actions task execution (task 3.5) | One `RunOnce` verification pass, plus a small number of directory/test objects | Task execution: $0.25 each; objects targeted: $0.10/1M; objects operated on: $0.01/1M; task-operations-invoked: $1.00/1M | One execution plus well under 1M objects: `$0.25 + (<100 × $0.10 / 1,000,000) + (<100 × $0.01 / 1,000,000) ≈ $0.25` |
...
|---|---:|---|
| **One delivery, minimum modeled foundation** | **$0.391** | Three-hour environment: 3 hours of the configured share ($0.068808), 15 minutes of the verification VM ($0.066250), 3 hours of its 32 GiB OS disk ($0.006312), and one Storage Actions verification execution ($0.25). This assumes the deployment script deallocates the VM after its in-network setup. It excludes private endpoints, payload/operations, and every unsized workload service. |
| **Idle per day, minimum modeled foundation** | **$0.601/day** | Configured provisioned-v2 SSD share: `24 × $0.022936 = $0.550464`; 32 GiB OS disk: `24 × $0.002104 = $0.050496`. The share dominates this bounded total and bills even with no data activity; the Storage Actions task itself has no idle charge between runs. |
