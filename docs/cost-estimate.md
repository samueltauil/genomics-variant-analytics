...
**Date retrieved:** September 21, 2026. **Region:** `eastus2`. **Currency:** USD.
...
Purview, Slurm/Managed Lustre, and the task 8.7 governed-query proof to be
attempted). The September 21 governed-query proof reused the already-modeled
verification VM rather than adding a separate warehouse, notebook, or SQL
service SKU; once the VM was deallocated again, the query surface carried no
incremental standing Azure charge.
...
end-to-end accelerator: Batch/HPC, Managed Lustre, a Delta engine, separate
analytics capacity beyond the verification VM-hosted task 8.7 service, genomic
payload retention, data transfer, transactions, staging
...
| Governed query engine (task 8.7) | Python/SQLite HTTP service installed by `infra/governed-query.bicep` on the existing verification VM | No separate Azure meter beyond the verification VM and OS disk already listed above | `$0` incremental idle cost after deallocation; delivery cost is the verification-VM runtime already counted above. |
...
## Purview: live-tested, then torn down again (task 3.4 remains blocked)
...
An authorized live attempt on September 21, 2026 cleared the original
readiness blockers: `Microsoft.Purview` was registered, the CLI extension was
installed, and a short-lived private account plus temporary account/platform
private endpoints, DNS zones, and Data Factory managed private endpoints were
deployed from `infra/purview-lineage-live.bicep`. Two real Copy runs then
succeeded, but the second activity output still reported
`reportLineageToPurview.status = "CannotConnect"` / `The catalog provided for
lineage reporting cannot be connected, please check the catalog uri and network
connection.` Matching Atlas lookups over the private endpoint from the
verification VM never produced the expected staged sink asset or `adf_copy_activity`
entity, so task 3.4 remains unchecked for a live product-integration reason,
not because the environment was unprepared. The Purview account, Purview-only
private endpoints/DNS, and temporary Data Factory Purview wiring were deleted
immediately after evidence capture, so the standing ~$9.86/day charge is not
left running in the disposable environment.
...
| **Forecast for the `20260919` run against the $500 ceiling** | **≈ $0.14 delivery + $0.601/day idle** | At $0.601/day, the modeled foundation could stay up roughly 831 days before reaching $500 on idle storage cost alone; the ceiling is not at risk from this foundation while Purview stays undeployed. Task 8.7's live governed-query proof added no separate standing SKU and no incremental idle charge after the VM was deallocated. Future HPC/Batch/Lustre/Purview work still needs its own forecast before it runs and must stay within the remaining headroom below $500 total. |
