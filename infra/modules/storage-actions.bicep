...
resource task 'Microsoft.StorageActions/storageTasks@2023-01-01' = {
  ...
  properties: {
    enabled: true
    description: 'Tiers staged genomic artifacts under Ingest/ from Hot to Cool once Last-Modified precedes the configured threshold. Index tags are not applied: the lake account is HNS-enabled (ADLS Gen2) and blob index tags are unsupported on hierarchical-namespace accounts.'
    action: {
      if: {
...
resource assignment 'Microsoft.Storage/storageAccounts/storageTaskAssignments@2024-01-01' = {
  parent: lakeAccount
  name: '${name}run1'
  properties: {
    taskId: task.id
    enabled: true
    description: 'One-shot verification pass over staged Ingest/ artifacts in ${lakeFilesystem}.'
    executionContext: {
      target: {
        prefix: [
    ...
    report: {
      prefix: 'StorageTaskReports/${name}'
    }
  }
  dependsOn: [
    taskExecutionRole
