...
- [x] 8.7 Deploy with private endpoints and public data-plane endpoints disabled, and verify ingestion, staging, processing, and query all still function
  - Live verification (2026-09-21): the surviving `rg-genomics-20260919` environment still reports `publicNetworkAccess: Disabled`, `networkRuleSet.defaultAction: Deny`, and no public IPs. `infra/governed-query.bicep` deployed the run-command resource `/subscriptions/<authorized-subscription>/resourceGroups/rg-genomics-20260919/providers/Microsoft.Compute/virtualMachines/vm-genomics-20260919/runCommands/install-governed-query-service`, which installs a private governed query service on `/subscriptions/<authorized-subscription>/resourceGroups/rg-genomics-20260919/providers/Microsoft.Compute/virtualMachines/vm-genomics-20260919`. `scripts/Test-Environment.ps1` remained the live evidence for ingestion, staging, and processing; `scripts/Test-GovernedQuery.ps1` then uploaded the exact 20-field synthetic dataset to `https://stglaketgrxjnw6usjfg.blob.core.windows.net/healthcare/SampleData/Genomics/private-query/task-8-7/synthetic-governed-variants.csv`, recorded explicit reference build `GRCh38` plus manifest digest `manifest-sha256:synthetic-reference-001`, and executed the de-identified in-VNet query `SELECT CHROM, POS, ID, REF, ALT, QUAL, FILTER, INFO, sample_id, cohort_id, gene, transcript, variant_consequence, genotype, allele_frequency, reference_build, pipeline_version, source_file_uri, ingestion_timestamp FROM variant_records WHERE gene = ? ORDER BY cohort_id, sample_id, POS` through the governed service on `vm-genomics-20260919`. The live result returned 3 `GENE1` rows across `SYN-COHORT-A` and `SYN-COHORT-B`; a follow-up request asking for subject linkage was refused with HTTP 403 and `subject linkage is not available on the de-identified governed query interface`; and an outside-the-VNet request to the VM private endpoint target (`10.42.1.4:8080`) was refused from the operator workstation. The service audit trail recorded both the successful de-identified query and the denied subject-linkage attempt. The captured result set, refusal result, audit rows, VM private IP, and redacted resource IDs are committed at [evidence/private-query-live-2026-09-21.json](evidence/private-query-live-2026-09-21.json).
